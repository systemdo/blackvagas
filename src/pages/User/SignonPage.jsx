...
class SignonPage extends Component {
  ...
  validEmail = () => {
    ...
    if (email === "") {
      ...
      this.setState({
        showErrorEmail: true,
        helperTextEmail: "Email inválido"
      });
      return false;
    } else if (emailConfirm === "") {
      this.setState({
        showErrorEmailConfirm: true,
        helperTextEmailConfirm: "Email de confirmação não pode ser vazio"
      });
      return false;
    } else if (email !== emailConfirm) {
      this.setState({
        showErrorEmailConfirm: true,
        helperTextEmailConfirm: "Email de confirmação é diferente do email"
      });
      return false;
    } else if (!validateEmail(emailConfirm)) {
      this.setState({
        showErrorEmailConfirm: true,
        helperTextEmailConfirm: "Email inválido"
      });
      return false;
    }
    return true;
  ...
  validPassword = () => {
    ...
    if (password === "") {
      this.setState({
        ...
        helperTextPassword: "A senha não pode ser vazia"
      });
      return false;
    } else if (passwordConfirm === "") {
      this.setState({
        showErrorPasswordConfirm: true,
        helperTextPasswordConfirm: "A confirmação de senha não pode ser vazia"
      });
      return false;
      ...
      this.setState({
        showErrorPasswordConfirm: true,
        helperTextPasswordConfirm: "A confirmação de senha é diferente da senha"
      });
      return false;
    }
    return true;
  ...
  validCarrer = () => {
    ...
    if (carrer === "") {
      this.setState({
        showErrorCarrer: true,
        helpTextCarrer: "Qual sua Profissão?"
      });
      return false;
    }
  ...
  signOn = () => {
    if (this.validEmail() && this.validPassword() && this.validCarrer()) {
      ...
      this.UserService.doCreateUserWithEmailAndPassword(
        this.state.email,
        this.state.password
      )
        .then(res => {
      ...
      >
        ...
        <Grid container item xs={12} md={6}>
          <Paper className={classes.paper}>
            ...
            />
            <SelectSugestion
              handleSelected={this.handleCareer}
              data={data}
              showError={showErrorCarrer}
              helperText={helpTextCarrer}
            />
            <Button
              variant="contained"
